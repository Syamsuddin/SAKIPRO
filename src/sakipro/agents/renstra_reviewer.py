...
class RenstraReviewAgent(BaseAgent):
    agent_name = "Renstra Reviewer"
    supported_commands = ["/review-renstra"]
    
    def plan(self, request: Dict[str, Any]) -> Dict[str, Any]:
        user_query = request.get("query", "").strip()
        core_keywords = "Renstra Rencana Strategis Dinas Badan Tujuan Sasaran Indikator Kinerja"
    ...
    def run(self, request: Dict[str, Any]) -> Generator[str, None, None]:
        ...
        system_prompt = (
            ...
            "Anda adalah Asisten SAKIP yang ahli dalam meninjau Rencana Strategis (Renstra).\n"
            "Tugas Anda:\n"
            "1. Evaluasi keselarasan antara Visi, Misi, Tujuan, dan Sasaran Strategis.\n"
            "2. Pastikan indikator sasaran berorientasi pada hasil (outcome) atau dampak (impact).\n"
            "3. Pastikan untuk selalu menyertakan '[DRAFT: Harap periksa ulang]' di akhir jawaban Anda.\n\n"
            f"{context['context_text']}"
        )
        
        user_message = request.get("query", "Tolong review Renstra berdasarkan dokumen terkait.")
